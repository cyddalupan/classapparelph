...
<div class="create-user-page">
    ...
    <div class="page-header">
        <div class="header-content">
            ...
            <p class="page-subtitle">Add a new user with role-based permissions</p>
        </div>
                    ...
                    <div class="form-column">
                        ...
                        <div class="form-group">
                            ...
                            <div style="display:flex;align-items:stretch;">
                                <input type="text" id="username" name="username" class="form-control @error('username') is-invalid @enderror"
                                       value="{{ old('username') }}" placeholder="hal. aja"
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" id="position" name="position" class="form-control @error('position') is-invalid @enderror"
                                   value="{{ old('position') }}" placeholder="e.g., C.E.O., Manager, Owner">
                            ...
                            <select id="role" name="role" class="form-control @error('role') is-invalid @enderror" required onchange="toggleRoleFields()">
                                ...
                                <option value="sales_agent" {{ old('role') === 'sales_agent' ? 'selected' : '' }}>Sales Agent</option>
                                <option value="sales_representative" {{ old('role') === 'sales_representative' ? 'selected' : '' }}>Sales Representative</option>
                                ...
                                <option value="cpo" {{ old('role') === 'cpo' ? 'selected' : '' }}>C.P.O.</option>
                                <option value="cmo" {{ old('role') === 'cmo' ? 'selected' : '' }}>C.M.O.</option>
                                <option value="prod_manager" {{ old('role') === 'prod_manager' ? 'selected' : '' }}>Production Manager</option>
                                <option value="ga" {{ old('role') === 'ga' ? 'selected' : '' }}>G.A. / Agent</option>
                                <option value="qa" {{ old('role') === 'qa' ? 'selected' : '' }}>Q.A.</option>
                    ...
                    <div class="form-column">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" id="phone" name="phone" class="form-control @error('phone') is-invalid @enderror" 
                                   value="{{ old('phone') }}" placeholder="e.g., +63 912 345 6789">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" id="department" name="department" class="form-control @error('department') is-invalid @enderror" 
                                   value="{{ old('department') }}" placeholder="e.g., Sales">
                            @error('department') <span class="invalid-feedback">{{ $message }}</span> @enderror
                        </div>

                        <!-- Sales-specific fields (hidden for non-sales roles) -->
                        <div id="salesFields" style="display:none;">
                            ...
                            <div class="form-group">
                                ...
                                <input type="number" id="sales_target" name="sales_target" class="form-control @error('sales_target') is-invalid @enderror" 
                                       value="{{ old('sales_target') }}" step="0.01" min="0" placeholder="e.g., 100000">
                            ...
                            <div class="form-group">
                                ...
                                <input type="number" id="commission_rate" name="commission_rate" class="form-control @error('commission_rate') is-invalid @enderror" 
                                       value="{{ old('commission_rate') }}" step="0.1" min="0" max="100" placeholder="e.g., 5">
...
const roleDescriptions = {
    admin: 'Full access to all system features. Can manage users, settings, and view all data.',
    sales_agent: 'Can add and view own sales. Read-only view of status and payment verification.',
    sales_representative: 'Same as Sales Agent. Can add and view own sales with minimal fields.',
...
function toggleRoleFields() {
    ...
    // Show/hide sales-specific fields
    salesFields.style.display = (role === 'sales_agent' || role === 'sales_representative') ? 'block' : 'none';

    // Update role description
    if (role && roleDescriptions[role]) {
        desc.innerHTML = `<p><strong>${role.charAt(0).toUpperCase() + role.slice(1).replace('_', ' ')}:</strong> ${roleDescriptions[role]}</p>`;
    } else {
        desc.innerHTML = '<p>Select a role to see its description.</p>';
    }
}
