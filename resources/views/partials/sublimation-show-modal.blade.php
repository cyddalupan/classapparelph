...
window.sub_renderSpecs = function() {
    ...
    if (matchingGroup) {
        ...
        // Polo conditional: Button Color / Zipper Color
        // (parity with the create page's full sublimation modal)
        var matchedKey = null;
        for (var mk in sub_garmentSpecMap) {
            if (sub_garmentSpecMap.hasOwnProperty(mk) && sub_garmentSpecMap[mk] === matchingGroup) { matchedKey = mk; break; }
        }
        if (matchedKey === 'polo' || matchedKey === 'polo-zipper-cc' || matchedKey === 'polo-button-cc') {
            var isButton = upperName.indexOf('BUTTON') !== -1;
            var bzLabel = isButton ? 'Button Color' : 'Zipper Color';
            var bzId = isButton ? 'sub_spec_buttonColor' : 'sub_spec_zipperColor';
            var bzRowId = isButton ? 'subSpecRow_buttonColor' : 'subSpecRow_zipperColor';
            html += '<div class="d-flex align-items-center mb-2 p-2 rounded" style="background:#f8f9fa;border:1px solid #e9ecef;" id="' + bzRowId + '">';
            html += '<label class="form-label small fw-semibold mb-0 me-2" style="min-width:110px;white-space:nowrap;">' + bzLabel + '</label>';
            html += '<div class="flex-grow-1"><select class="form-control form-control-sm sub-spec-select" id="' + bzId + '" data-fid="' + bzId + '">';
            html += '<option value="">-- Select --</option><option value="BLACK">BLACK</option><option value="WHITE">WHITE</option>';
            html += '</select></div></div>';
        }
        ...
        // Wire up button/zipper color selectors (no auto-addon, just recompute)
        ['sub_spec_buttonColor', 'sub_spec_zipperColor'].forEach(function(bzId2) {
            var bzEl = container.querySelector('.sub-spec-select[data-fid="' + bzId2 + '"]');
            if (bzEl) bzEl.onchange = function () { sub_calculateTotal(); sub_autoGenerateDescription(); };
        });
    } else {
