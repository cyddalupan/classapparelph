                ...
                <div class="drop-zone" data-status="{{ $statusKey }}">
                    @forelse(($columns[$statusKey] ?? []) as $sale)
                        ...
                        <div class="kanban-card" data-id="{{ $sale->id }}" draggable="true" data-department="{{ $sale->department_id }}" data-photos="{{ $allPhotos ? 'ok' : 'missing' }}" data-balance="{{ $sale->balance_due_computed }}" data-priority="{{ $sale->priority ?? '' }}">
                            ...
                            @if($sale->priority)
                                @php $prioHue = max(0, min(45, (int) round(($sale->priority - 1) * 45 / (($priorityMax ?? 15) - 1)))); @endphp
                            ...
                            <div class="card-meta">
                                ...
                                @if(($sale->balance_due_computed ?? 0) > 0 && ($sale->net_paid ?? 0) > 0)
                                    <span style="display:inline-flex;align-items:center;gap:2px;padding:1px 6px;border-radius:4px;font-size:10px;background:#fff3cd;color:#664d03;font-weight:600;" title="May natitira pang bayad: ₱{{ number_format($sale->balance_due_computed, 2) }}">⚠️ Balance</span>
