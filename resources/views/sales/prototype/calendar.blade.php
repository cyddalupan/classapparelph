        ...
        <div class="col-lg-9">
            <div class="calendar-wrapper">
                ...
                <div class="range-bar" id="rangeBar">
                    ...
                    @if($canFilterAgent ?? false)
                    <label class="agent-filter" title="Filter by sales agent" style="display:inline-flex;align-items:center;gap:0.4rem;font-weight:600;font-size:0.85rem;color:#555;margin:0;">
                        <i class="fas fa-user-tie"></i> Agent:
                        <select id="calAgentFilter" onchange="applyCalAgentFilter()" style="padding:3px 8px;border-radius:6px;border:1px solid #dee2e6;font-size:0.8rem;max-width:170px;">
...
// Agent filter — populate dropdown mula sa mga naka-load na project (id → pangalan)
function populateAgentOptions() {
    var sel = document.getElementById('calAgentFilter');
    if (!sel) return;
    var map = {};
    (window.calProjects || []).forEach(function(p) {
        if (p.sales_agent_id != null && p.sales_agent_id !== '') {
            if (!map[p.sales_agent_id]) map[p.sales_agent_id] = p.sales_agent_name || ('Agent #' + p.sales_agent_id);
        }
    });
    ...
    ids.forEach(function(id) { html += '<option value="' + id + '">' + map[id] + '</option>'; });
    sel.innerHTML = html;
    if (cur && map[cur]) { sel.value = cur; }
    else { sel.value = ''; calFilters.agent = ''; }
}
function applyCalAgentFilter() {
    var sel = document.getElementById('calAgentFilter');
    calFilters.agent = sel ? sel.value : '';
    applyCalFilters();
}
        ...
        if (dayProjects.length === 0) {
            ...
            dayProjects.forEach(p => {
                ...
                html += `<div class="day-project ${isMoved?'moved':''}${splitChunk?' is-split':''}" style="background:${color}15;border-left:3px solid ${splitChunk ? '#e67e22' : (isMoved ? '#fd7e14' : color)};"
                    draggable="${(IS_SALES_AGENT || IS_GA || IS_QA || splitChunk) ? 'false' : 'true'}" data-id="${p.id}" data-split="${splitChunk ? '1' : '0'}" data-prio="${p.priority || ''}" data-agent="${p.sales_agent_id || ''}" data-g1="${pt.g1}" data-g2="${pt.g2}" data-g3="${pt.g3}" data-garments="${bd.garments.join(',')}" data-fabrics="${bd.fabrics.join(',')}" data-parts="${bd.parts.join(',')}" data-stage="${(p.production_stage || STATUS_TO_STAGE[p.kanban_status] || 'HOLD').replace(/"/g,'&quot;')}" onclick="${(IS_SALES_AGENT || IS_GA || IS_QA) ? '' : `showDetail(${p.id})`}" title="${name}${splitChunk ? ' — ✂ araw ' + splitIdx + '/' + splitTotal : ''}${(IS_SALES_AGENT || IS_GA) ? '' : ' - ' + curr(amt)}">`;
                if (isMoved) {
...
document.addEventListener('dragstart', function(e) {
    if (IS_SALES_AGENT || IS_GA || IS_QA) return;
...
var calFilters = { prio: false, garments: {}, fabrics: {}, parts: {}, stage: {}, agent: '' };
...
function getProjectGarmentTotals(p) {
    ...
    (p.services || []).forEach(function(it) {
        ...
        else if (GARMENT_GROUP2.indexOf(g) !== -1) g2 += qty;
        else g3 += qty;
    });
...
function applyCalFilters() {
    document.querySelectorAll('#weekContainer .day-project').forEach(function(card) {
        ...
        if (show && calFilters.agent) {
            if (String(card.getAttribute('data-agent') || '') !== String(calFilters.agent)) show = false;
        }
        ...
        if (show && hasActive(calFilters.stage)) {
            var stl = (card.getAttribute('data-stage') || '').split(',');
            if (!matchesAny(calFilters.stage, stl)) show = false;
        }
...
function getFilteredProjects() {
    ...
    return window.calProjects.filter(function(p) {
        ...
        if (calFilters.agent) {
            if (String(p.sales_agent_id || '') !== String(calFilters.agent)) return false;
        }
        ...
        if (hasActive(calFilters.stage)) {
            var pStage = p.production_stage || STATUS_TO_STAGE[p.kanban_status] || 'HOLD';
            if (!calFilters.stage[pStage]) return false;
        }
        return true;
    });
}
...
function updateSummary(projects) {
    ...
    let html = `
        <div class="d-flex justify-content-around mb-3 pb-2 border-bottom">
            ...
            ${(IS_SALES_AGENT || IS_GA) ? '' : `<div class="summary-stat"><div class="stat-number" style="color:#28a745;">${curr(totalAmt)}</div><div class="stat-label">Value</div></div>`}
        </div>`;
    ...
    stageOrder.forEach(st => {
        if (stb[st]) {
            ...
            html += `<div class="summary-item summary-item-compact summ-toggle" data-f-type="stage" data-f-val="${st.replace(/"/g,'&quot;')}"><span><span style="display:inline-block;width:8px;height:8px;border-radius:50%;background:${c};margin-right:0.4rem;"></span>${st}</span><span class="value">${stb[st].count} proj · ${stb[st].qty} pc${stb[st].qty>1?'s':''}</span></div>`;
        }
    });
...
function showDetail(id) {
    ...
    var fbBodyC = document.getElementById('modalFreebieSlipBody');
    if (prodBody) prodBody.dataset.loaded = '';
    if (addProdBody) addProdBody.dataset.loaded = '';
    if (fbBodyC) fbBodyC.dataset.loaded = '';
    ...
    if (fbBodyC) fbBodyC.style.display = 'none';
...
document.addEventListener('click', function(e) {
    ...
    if (tabName === 'details') {
        ...
        if (fbBody) {
            ...
            if (psSaleId && (!fbBody.dataset.loaded || fbBody.dataset.saleId !== String(psSaleId))) {
                loadFreebieSlipTab(psSaleId);
            }
        }
    } else {
        detailsBody.style.display = 'none';
    ...
    slips.forEach(function(slip, pIdx) {
        ...
        if (hasRoster && allRosters.length > 0) {
            ...
            caSortedIdxBySize(allRosters, caRowSize).forEach(function(origIdx, dispIdx) {
                ...
                html += '<td style="text-align:center;">' + psQaCell(saleId, itemIdx, 'qa1_done', (items[itemIdx] && items[itemIdx].qa1_done), (items[itemIdx] && items[itemIdx].qa1_count) || '', 'main', false) + '</td>';
                html += '<td style="text-align:center;">' + psQaCell(saleId, itemIdx, 'qa2_done', (items[itemIdx] && items[itemIdx].qa2_done), (items[itemIdx] && items[itemIdx].qa2_count) || '', 'main', false) + '</td>';
                html += '</tr>';
            });
            html += '</tbody></table>';
        ...
        data.products.forEach(function(prod, idx) {
            ...
            if (hasRoster) {
                ...
                caSortedIdxBySize(rosterData, caRowSize).forEach(function(origRi, dispRi) {
                    ...
                    html += '<td style="text-align:center;">' + psQaCell(saleId, rowItem, 'qa1_done', rowDone.qa1_done, rowDone.qa1_count || '', 'add', rowItem < 0) + '</td>';
                    html += '<td style="text-align:center;">' + psQaCell(saleId, rowItem, 'qa2_done', rowDone.qa2_done, rowDone.qa2_count || '', 'add', rowItem < 0) + '</td>';
                    html += '</tr>';
                });
                html += '</tbody></table>';
            } else if (sizes.length > 0) {
                ...
                caSortedIdxBySize(sizes, caRowSize).forEach(function(origSi, dispSi) {
                    ...
                    html += '<td style="text-align:center;">' + psQaCell(saleId, sItem, 'qa1_done', sDone.qa1_done, sDone.qa1_count || '', 'add', sItem < 0) + '</td>';
                    html += '<td style="text-align:center;">' + psQaCell(saleId, sItem, 'qa2_done', sDone.qa2_done, sDone.qa2_count || '', 'add', sItem < 0) + '</td>';
                    html += '</tr>';
                });
                html += '</tbody></table>';
            } else {
...
// QA1/QA2 cell: checkbox + maliit na number box ("piraso na tapos"). Additive lang.
function psQaCell(saleId, idx, doneField, done, count, scope, rowDisabled) {
    var sc = scope || 'main';
    var rowDis = rowDisabled ? ' disabled' : '';
    var numField = doneField.replace(/_done$/, '_count');
    var val = (count === 0 || count) ? count : '';
    var numDis = (!psCanEdit || rowDisabled) ? ' disabled' : '';
    return '<span style="display:inline-flex;align-items:center;gap:3px;justify-content:center;">'
        + '<input type="checkbox" onchange="toggleProdCheck(' + saleId + ', ' + idx + ', \'' + doneField + '\', this.checked, \'' + sc + '\')" ' + (done ? 'checked' : '') + rowDis + '>'
        + '<input type="number" min="0" step="1" value="' + val + '" title="Piraso na tapos" style="width:46px;padding:1px 3px;font-size:11px;text-align:center;" onchange="saveProdCount(' + saleId + ', ' + idx + ', \'' + numField + '\', this.value, \'' + sc + '\')"' + numDis + '>'
        + '</span>';
}

// Itala ang bilang ng tapos (QA1/QA2) — parehong items JSON.
function saveProdCount(saleId, index, field, value, scope) {
    ...
    .then(function(data) { if (!data.success) console.error('Failed to save count', data); })
