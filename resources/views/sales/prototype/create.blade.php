...
<style>
    ...
    /* Quick mode: only show Sizes and Pricing tabs */
    #productModal.sublimation-quick-mode #sublimationTab .nav-item:not(.tab-sizes):not(.tab-pricing) {
        display: none !important;
    }
    #productModal.sublimation-quick-mode #order-details,
    #productModal.sublimation-quick-mode #specs {
        display: none !important;
    }
</style>
@endpush
...
window.openProductModal = function(productType) {
    ...
    // Clean up quick-mode from previous fullsublimation modal usage
    var pm = document.getElementById('productModal');
    if (pm) pm.classList.remove('sublimation-quick-mode');
...
window.sublimation_openModal = function() {
    // Activate quick mode: hide Order Details and Specifications tabs
    var modal = document.getElementById('productModal');
    if (modal) modal.classList.add('sublimation-quick-mode');
    
    // Auto-fill redundant fields with defaults
    var today = new Date();
    var dateStr = today.getFullYear() + '-' + String(today.getMonth()+1).padStart(2,'0') + '-' + String(today.getDate()).padStart(2,'0');
    
    var projEl = document.getElementById('sublimation_projectName');
    var dateEl = document.getElementById('sublimation_dateNeeded');
    var designerEl = document.getElementById('sublimation_designer');
    if (projEl) projEl.value = 'FS'; // will be updated after garment selection
    if (dateEl) dateEl.value = dateStr;
    if (designerEl) designerEl.value = 'WALK-IN';
    
    // Reset ALL form fields
    var fieldsToReset = [
        'sublimation_description',
        'sublimation_notes'
    ];
    ...
    // Switch to Sizes tab (quick mode)
    var sizesTab = document.querySelector('#sizes-tab');
    if (sizesTab && bootstrap && bootstrap.Tab) {
        var tab = new bootstrap.Tab(sizesTab);
        tab.show();
    }
    ...
    fetch('/api/sublimation-prices')
        ...
        .then(function(data) {
            ...
            // In quick mode: auto-select first garment & fabric
            if (modal && modal.classList.contains('sublimation-quick-mode')) {
                if (data.garments && data.garments.length > 0) {
                    garmentSelect.value = data.garments[0].id;
                    var event = new Event('change');
                    garmentSelect.dispatchEvent(event);
                    // Update project name with first garment
                    if (projEl) projEl.value = 'FS: ' + data.garments[0].name;
                }
                if (data.fabrics && data.fabrics.length > 0) {
                    fabricSelect.value = data.fabrics[0].id;
                    var ev2 = new Event('change');
                    fabricSelect.dispatchEvent(ev2);
                }
            }
